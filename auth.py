"""
Асинхронная аутентификация Predict Fun API
"""
...
async def get_auth_jwt(
    ...
    proxy_url = format_proxy_for_aiohttp(proxy) if proxy else None

    def _sync_auth():
        import requests
        from predict_sdk import OrderBuilder, ChainId, OrderBuilderOptions
        from config import format_proxy
        ...
        if not msg_resp.ok:
            raise Exception(f"Ошибка получения сообщения: {msg_resp.status_code}")
        message = msg_resp.json()["data"]["message"]
        signature = builder.sign_predict_account_message(message)
        body = {"signer": predict_account_address, "message": message, "signature": signature}
        ...
        if not jwt_resp.ok:
            raise Exception(f"Ошибка JWT: {jwt_resp.status_code}")
